...
### 设计模式之抽象工厂模式
##### 一、定义
	为创建一组相关或相互依赖的对象提供一个接口，而且无需指定他们的具体类。它是创建型模型。	
##### 二、模型
![抽象工厂模式](抽象工厂模式.JPG)
#####  三、解析
**==优点:==**
...
### 设计模式之工厂方法模式
##### 一、定义
		工厂方法模式（FACTORY METHOD）是一种常用的对象创建型设计模式，此模式的核心精神是封装类中不变的部分，提取其中个性化善变的部分为独立类，通过依赖注入以达到解耦、复用和方便后期维护拓展的目的。它的核心结构有四个角色，分别是抽象工厂；具体工厂；抽象产品；具体产品。它是创建型模型。
##### 二、模型
![工厂方法模式](工厂方法模式.JPG)
#####  三、解析
**==优点:==**
...
### 设计模式之单例模式
##### 一、定义
		应用该模式的类一个类只有一个实例。即一个类只有一个对象实例。它是创建型模型。
==单例模式有三种：饿汉式、懒汉式和登记式。==
##### 二、模型
![单例模式](单例模式.png)
#####  三、解析
...
### 设计模式之观察者模式
##### 一、定义
		定义对象间的一种一对多的依赖关系，当一个对象的状态发生改变时，所有依赖于它的对象都得到通知并被自动更新。它是行为型模型。
##### 二、模型
![观察者模式.](观察者模式.png)
#####  三、解析
**==优点：==** 
...
### 设计模式之策略模式
##### 一、定义 
	一个类的行为或其算法可以在运行时更改。具体来说就是对象有某个行为，但是在不同的场景中，该行为有不同的实现算法。它是行为型模型。
##### 二、模型
![策略模式](策略模式.png)
##### 三、解析
**==优点： #0bee0b==**
...
### 设计模式之模板模式
##### 一、定义 
	定义一个模板结构，将具体内容延迟到子类去实现。它是行为型模型。
##### 二、模型
![模板模式](模板模式.JPG)
##### 三、解析
**==主要作用：==**

		可以在不改变模板结构的前提下，在其子类中重新定义模板中的行为。
**==优点： #0bee0b==**

		1） 提高代码复用性
    		将相同部分的代码放在抽象的父类中，便于维护。
		2）  提高了拓展性
			将不同的代码放入不同的子类中，通过对子类的扩展增加新的行为。
		3）  实现了反向控制
			通过一个父类调用其子类的操作，通过对子类的扩展增加新的行为（行为又父类控制、子类实现），实现了反向控制 & 符合“开闭原则”。
**==缺点： #fc0606==**
...
/*模板匹配*/
public class TemplateMatching extends  ObjectLocation{
	/*图像预处理*/
	void ImgPreproccess(){
		/*图像预处理方法1*/
	}
	...
	void Locating(){
		/*基于边缘轮廓的模板匹配定位方法*/
		System.out.println("模板匹配定位(X, Y)：");
	}
}

/*机器学习*/
public class MachineLearning extends  ObjectLocation{
	/*图像预处理*/
	void ImgPreproccess(){
		/*图像预处理方法2*/
	}
...
public class MyDo{ 
	public static void main(String[] args){ 
		//模板匹配定位
		ObjectLocation moselLctn = new TemplateMatching();
		moselLctn.ExportLocation();
		
		//机器学习定位
		moselLctn = new MachineLearning();
		moselLctn.ExportLocation();
	}
}
```
### 设计模式之适配器模式
##### 一、定义 
		适配器模式：将一个类的接口转换成客户希望的另一个接口。适配器模式让那些接口不兼容的类可以一起工作。它是结构型模型。
##### 二、模型
![适配器模式](适配器模式.JPG)
##### 三、解析
**==何时使用：==**

		1、系统需要使用已有的类，但是这个类的接口不符合系统的要求。 
		2、想要建立一个可以重复使用的类，用于与一些彼此之间没有太大关联的一些类，包括一些可能在将来引进的类一起工作，这些源类不一定有一致的接口。 
		030、通过接口转换，将一个类插入另一个类系中。
**==优点： #0bee0b==**

		1、将目标类和适配者类解耦，通过引入一个适配器类来重用现有的适配者类，而无需修改原有结构。以此实现两个没有关联的类一起运行。 
		2、提高了类的复用。
		3、增加了类的透明度。
		4、灵活性和扩展性良好。 
**==缺点： #fc0606==**

		1、过多地使用适配器，会让系统结构变得非常零乱，不易整体进行把握，增加了系统的复杂性。
		2.一次最多只能适配一个适配者类，不能同时适配多个适配者，而且目标类必须是抽象类。 
**==例子：==**

		场景：公司开发的安防系统中，原本是通过公司派发给访客临时进出证(带电磁感应）时刻追踪客户在公司办公场地中的位置的，线下由于人工智能的发展及落地，需要在不影响原有功能及系统结构的情况下，通过机器学习的方法来追踪访客，其中机器学习的方式又有两种方式，人脸及人形追踪，并且这两种应用已经被另一个团队封装为一个现成的组件。
		现使用适配器模式设计一个系统，实现需求。
...
/*************************************************************************************************************/
/*人工智能追踪接口*/
public interface IAITracker {
	public void AITracking();
}

/*人脸追踪类*/
public class FaceTracker implements IAITracker{
	@Override
	public void AITracking(){
		/*
		    人脸追踪
...
/*人形追踪*/
public class FigureTacker implements IAITracker{
	@Override
	public void AITracking(){
		/*
		    人型追踪
...
public class TrackerAdapter implements ITracker{
	@Autowired
	private IAITracker aiTracker;
	
	@Override
	public void tracking(String type){
		if(type.equalsIgnoreCase("face")){
			aiTracker = new FaceTracker();
		}else if(type.equalsIgnoreCase("figure")){
			aiTracker = new FigureTacker();
		}
		aiTracker.AITracking();
	}
}
	
/*原来的 ITracker 接口的实体类*/
public class InductionTracker implements ITracker{

	@Override
	public void tracking(String type){
		if(type.equalsIgnoreCase("induction")){
			System.out.println("************电磁追踪************");
		}
	}
}
/*************************************************************************************************************/
...
public class AdapterClient{ 
	public static void main(String[] args){ 
		ITracker tracker;
		tracker = new InductionTracker();
		tracker.tracking("induction");
		tracker = new TrackerAdapter();
		tracker.tracking("face");
		tracker.tracking("figure");
	}
}
```